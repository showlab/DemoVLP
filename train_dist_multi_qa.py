...
from trainer import Multi_ObjectQATrainer_dist
...
def run():
    ...
    if config['visualizer']['type'] != "":
        visualizer = config.initialize(name='visualizer',
                                       module=module_vis,
    ...
    print('Val dataset: ', [x.n_samples for x in valid_data_loader],
          ' samples')
    ...
    if 'lr_scheduler' in config._config:
        if hasattr(transformers, config._config['lr_scheduler']['type']):
            lr_scheduler = config.initialize('lr_scheduler', transformers,
                                             optimizer)
        else:
    ...
    if config['trainer']['neptune']:
        ...
        writer = None
    trainer = Multi_ObjectQATrainer_dist(
        args,
        model,
        loss,
        metrics,
...
def init_dataloaders(config, module_data):
    ...
    if "type" in config["data_loader"] and "args" in config["data_loader"]:
        ...
        if config['data_loader']['args']['dataset_name'].startswith(
                "Conceptual"):
            config['data_loader']['args'] = replace_nested_dict_item(
                config['data_loader']['args'], 'subsample', 0.01)
