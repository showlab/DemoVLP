...
def t2v_metrics(sims, query_masks=None):
    ...
    if rows.size > num_queries:
        ...
        if break_ties == "optimistically":
            ...
            if False:
                print(
                    "Running slower code to verify rank averaging across ties")
    ...
    if query_masks is not None:
        ...
        assert cols.size == query_masks.sum(
        ), "masking was not applied correctly"
...
def evaluate_qa(results, label2ans, qid2data):
    ...
    answer_type2idx = {
        ...
        for idx, k in enumerate([
            "what", "who", "how", "where", "when", "object", "number", "color",
            "location"
        ])
    }
    ...
    for ans_type, ans_type_idx in answer_type2idx.items():
        ...
        print(ans_type, np.sum(answer_type_corrects),
              len(answer_type_corrects))
        metrics[f"{ans_type}_acc"] = float(np.mean(
            answer_type_corrects)) if len(answer_type_corrects) != 0 else 0
