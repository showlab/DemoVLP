...
from trainer import Multi_ObjectMCTrainer_dist
...
def run():
    ...
    if config['trainer']['neptune']:
        ...
        writer = None
    trainer = Multi_ObjectMCTrainer_dist(
        args,
        model,
        loss,
        metrics,
...
def init_dataloaders(config, module_data):
    ...
    if "type" in config["data_loader"] and "args" in config["data_loader"]:
        ...
        if config['data_loader']['args']['dataset_name'].startswith("LSMDC"):
