...
class LSMDCObjectSelect(TextObjectDataset):
    ...
    def _load_metadata(self):
        metadata_dir = os.path.join(self.data_dir, "meta_data")
        split_files = {
            'train': 'LSMDC16_annos_training.csv',
            'val': 'LSMDC16_challenge_1000_publictect.csv',  # there is no test
    ...
    def _get_object_path(self, sample):
        ...
        remove = sub_path.split('_')[-1]
        sub_path = sub_path.replace('_' + remove, '/')
        rel_object_fp = sub_path + video_fp
...
class LSMDCMCObjectSelect(TextObjectDataset):
    ...
    def _load_metadata(self):
        ...
        split_files = {
            ...
            'val':
            'LSMDC16_multiple_choice_test_randomized.csv',  # there is no test
