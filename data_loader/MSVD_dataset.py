...
class MSVDObjectSelect(TextObjectDataset):
...
class MSVDQAObjectSelect(TextObjectDataset):
    ...
    def _load_metadata(self):
        ...
        answer_set_file = os.path.join(self.metadata_dir,
                                       "msvd_answer_set.txt")
        mapping_file = os.path.join(self.metadata_dir,
                                    "msvd_youtube_mapping.txt")
        self.ans2label = {}
        self.vid2link = {}
        with open(answer_set_file, 'r') as f:
            lines = f.readlines()
            for idx, label in enumerate(lines):
                self.ans2label[label.strip()] = idx

        with open(mapping_file, 'r') as f:
            lines = f.readlines()
            for line in lines:
                link, vid = line.strip().split(" ")
                vid = int(vid[3:])
                self.vid2link[vid] = link
        ...
        raw_datalist = load_json(meta_file)
        ...
        for raw_d in raw_datalist:
            d = dict(
                ...
                question_id=raw_d[
                    "id"],  # be careful, it is not unique across splits
                answer_type=raw_d["question"].split(" ")[0])
            datalist.append(d)
    ...
    def _get_video_path(self, sample):
        rel_video_fp = self.vid2link[sample["vid_id"]] + '.avi'
        full_video_fp = os.path.join(self.data_dir, rel_video_fp)
        return full_video_fp, rel_video_fp
    ...
    def _get_object_path(self, sample):
        ...
        Returns:
            abs path
        """
        rel_object_fp = self.vid2link[sample["vid_id"]]
        full_object_fp = os.path.join(self.object_dir, rel_object_fp)
        return rel_object_fp, full_object_fp
