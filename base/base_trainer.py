...
class Multi_BaseTrainer_dist:
    ...
    def __init__(self,
        ...
        if config.resume is not None or cfg_trainer["resume"] is not None:
            if config.resume is not None:
                resume = config.resume
            else:
                resume = cfg_trainer["resume"]
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        checkpoint = torch.load(resume_path,
                                map_location='cuda:{}'.format(self.args.rank))
        ...
        redo_dp = False
        if not curr_state_dict_keys[0].startswith(
                'module.') and load_state_dict_keys[0].startswith('module.'):
            undo_dp = True
        elif curr_state_dict_keys[0].startswith(
                'module.'
        ) and not load_state_dict_keys[0].startswith('module.'):
            redo_dp = True
            undo_dp = False
        else:
            undo_dp = False
